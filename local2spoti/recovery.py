        ...
        for fid, path_str, _dur_ms in rows:
            ...
            try:
                await _process_one_file(
                    state, fid, path_str, statuses, processed, total,
                    outcomes=outcomes,
                    acoustid=acoustid, musicbrainz=musicbrainz,
                    songlink=songlink, spotify=spotify,
                )
...
async def _process_one_file(
    state: "AppState", fid: int, path_str: str,
    statuses: tuple[str, ...], processed: int, total: int,
    *, outcomes: dict[str, int],
    acoustid: AcoustidClient, musicbrainz: MusicBrainzClient,
    songlink: SongLinkClient, spotify: SpotifyClient | None = None,
) -> None:
    ...
    if spotify_track_id:
        await state.db_conn.execute(
            """UPDATE local_file SET
                ...
                metadata_source=?
               WHERE id=?""",
            (md.artist, md.title, isrc, spotify_track_id,
...
async def match_via_mb_text(
    ...
    if not rows:
        await state.bus.publish(ProgressEvent(
            stage="mb_text", processed=0, total=0,
    ...
    processed = 0
    await state.bus.publish(ProgressEvent(
        stage="mb_text", processed=0, total=total,
    ...
    if row:
        ...
        async def _provide() -> str:
            try:
                ...
                pass
            cur3 = await state.db_conn.execute(
                "SELECT access_token FROM auth_token WHERE key='spotify'"
            )
    ...
    try:
        for fid, path_str, artist, title, album, existing_isrc in rows:
            if state.cancel_event.is_set():
                await state.bus.publish(ProgressEvent(
                    stage="mb_text", processed=processed, total=total,
                    message="cancelled",
                ))
                return outcomes
            current_name = path_str.rsplit("/", 1)[-1][:60]
            ...
            if spotify_track_id:
                await state.db_conn.execute(
                    """UPDATE local_file SET
                        ...
                        metadata_source=?
                       WHERE id=?""",
                    (mb_res.isrc, spotify_track_id, match_method,
                     metadata_source, fid),
                )
                if match_method == "odesli":
                    outcomes["odesli"] += 1
        ...
        await state.bus.publish(ProgressEvent(
            stage="mb_text", processed=total, total=total,
            message=(
