...
from local2spoti.musicbrainz import MBResolution, MusicBrainzClient
...
async def test_follows_301_redirect_when_mbid_was_merged():
    ...
    respx.get(f"https://musicbrainz.org/ws/2/recording/{old_mbid}").mock(
        return_value=httpx.Response(
            301,
            headers={
                "Location": (f"https://musicbrainz.org/ws/2/recording/{new_mbid}?inc=url-rels&fmt=json"),
            },
        )
    )
