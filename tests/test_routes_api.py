    ...
    async with connect(db) as conn:
        ...
        for i in range(2):
            ...
            await repo.insert_candidates(
                ...
                [
                    MatchCandidate(
                        spotify_track_id=f"top{i}",
                        spotify_artist="Daft Punk",
                        spotify_title=f"Track {i}",
                    ...
                    MatchCandidate(
                        spotify_track_id=f"alt{i}",
                        spotify_artist="Daft Punk",
                        spotify_title=f"Track {i} (Live)",
                        artist_similarity=0.93,
...
async def test_approve_above_confidence_picks_only_files_at_or_above_threshold(tmp_path, monkeypatch):
    ...
    async with LifespanManager(app), AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        r = await c.post("/api/review/approve_above_confidence", data={"threshold": "0.85"})
    assert r.status_code == 200
    body = r.json()
    ...
    async with connect(db) as conn:
        cur = await conn.execute("SELECT id, status FROM local_file ORDER BY id")
...
async def test_approve_above_confidence_rejects_out_of_range(tmp_path, monkeypatch):
    ...
    app = create_app()
    async with LifespanManager(app), AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        r = await c.post("/api/review/approve_above_confidence", data={"threshold": "1.5"})
...
async def test_retry_errors_resets_files_and_clears_candidates(tmp_path, monkeypatch):
    ...
    async with connect(db) as conn:
        cur = await conn.execute("SELECT path, status, last_error FROM local_file ORDER BY path")
...
async def test_retry_one_error_resets_single_file(tmp_path, monkeypatch):
    ...
    async with connect(db) as conn:
        ...
        await conn.execute("UPDATE local_file SET last_error='boom' WHERE path IN ('/e1.mp3', '/e2.mp3')")
        await conn.commit()
